...
//update status cho nhan vien giao hàng
    ...
    public boolean updateOrderStatus(int orderId, String newStatus) {
       ...
       // xem l?ch su giao hang cho quan li
    ...
    //theo doi trang thai don hàng hco quan li
   ...
   public boolean assignSingleDelivery(int orderId) {
    ...
    // Bước 2: Cập nhật để gán nhân viên giao hàng, giới hạn tối đa 5 đơn hàng mỗi ngày
    String updateQuery = "UPDATE da "
            ...
            + "FROM DeliveryAssignments da "
            + "JOIN ( "
            + "    SELECT TOP 1 ss.employee_id, ss.shift_id "
            + "    FROM StaffSchedule ss "
            + "    WHERE ss.shift_date = CAST(GETDATE() AS DATE) "
            + "    AND ( "
            ...
            + "    AND ( "
            + "        SELECT COUNT(*) "
            + "        FROM DeliveryAssignments da2 "
            + "        WHERE da2.delivery_staff_id = ss.employee_id "
            + "        AND CAST(da2.assigned_at AS DATE) = CAST(GETDATE() AS DATE) "
            + "    ) < 6 " // Giới hạn 6 đơn hàng mỗi ngày cho mỗi nhân viên
            + "    ORDER BY NEWID() "
            + ") ss ON da.shift_id IS NULL "
            + "WHERE da.order_id = ? "
